...
class CartManager {
    ...
    async checkout() {
        ...
        // Check for issues that would prevent checkout
        const hasOwnProducts = this.cartProducts.some(product => product.createdBy === this.currentUser.uid);
        const hasOutOfStock = this.cartProducts.some(product => (product.stock || 0) === 0);
        const hasInsufficientStock = this.cartProducts.some(product => product.quantity > (product.stock || 0));
        ...
        if (hasOutOfStock) {
            this.showMessage('Some items are out of stock. Please remove them from cart.', 'error');
            return;
        }

        if (hasInsufficientStock) {
            this.showMessage('Some items have insufficient stock. Please update quantities.', 'error');
            return;
        }
