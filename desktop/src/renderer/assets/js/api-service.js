...
class ApiService {
    ...
    // New Order Management (Cart-to-Checkout Flow)
    async saveOrderFromCart(orderData) {
        return await this.apiCall('/orders/save-from-cart', 'POST', orderData);
    }
    ...
    async markOrderForCheckout(orderId) {
        return await this.apiCall(`/orders/${orderId}/checkout`, 'PATCH');
    }

    async markOrderAsPaid(orderId, buyerId, paymentDetails) {
        return await this.apiCall(`/orders/${orderId}/mark-paid`, 'PATCH', { buyerId, paymentDetails });
    }

    async cancelOrderById(orderId, buyerId, reason = '') {
        return await this.apiCall(`/orders/${orderId}/cancel`, 'PATCH', { buyerId, reason });
    }

    // Payment Management
    async getVendorPaymentConfig(vendorId) {
        return await this.apiCall(`/payments/vendor-config/${vendorId}`);
    }

    async updateVendorPaymentConfig(vendorId, config) {
        return await this.apiCall(`/payments/vendor-config/${vendorId}`, 'PUT', config);
    }

    async createRazorpayOrder(orderData) {
        return await this.apiCall('/payments/create-razorpay-order', 'POST', orderData);
    }

    async verifyRazorpayPayment(paymentData) {
        return await this.apiCall('/payments/verify-razorpay-payment', 'POST', paymentData);
    }
